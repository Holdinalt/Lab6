...
import server.sourse5Lab.collectionWorker.CollectionWrapper;
import server.sourse5Lab.exeptions.NoFileException;
import server.sourse5Lab.factories.IdGenerator;
...
public class FileManager implements IOInterface {
    ...
    public FileManager(CollectionWrapper c,String fileAddress) throws NoFileException {
        ...
        while(true){
            ...
            if (saveFile.exists() || saveFile.canRead() || saveFile.canWrite()){
                break;
            }
            System.out.println("Файл с коллекцией не найден или у вас нет достаточных прав, введите новый файл");
            fileAddress = new Scanner(System.in).nextLine();
        }
    }
